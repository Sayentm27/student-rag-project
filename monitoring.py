...
def check_hallucination(answer, context_docs):
    ...
    prompt = f"""
    You are a helpful assistant that evaluates the accuracy of a response based on the provided context.
    The context is a list of documents that were used to generate the response.
    The answer is the answer that was generated by the model.
    The verdict is the classification of the response as GROUNDED, PARTIAL, or HALLUCINATED.
    The verdict is based on the following criteria:
    - GROUNDED: The response is fully supported by the context.
    - PARTIAL: The response is partially supported by the context.
    - HALLUCINATED: The response is not supported by the context.
    The context is:
    {context}
    The answer is:
    {answer}
    Respond with exactly one word, the verdict: GROUNDED, PARTIAL, or HALLUCINATED based on the context and answer.
    """
